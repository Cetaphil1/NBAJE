  ...
  return (
    ...
    >
      ...
      <div style={{ padding:'16px 20px 20px', display:'flex', flexDirection:'column', gap:12 }}>
        ...
        {meta.socialProof && (
          <div style={{ background:'oklch(18% 0.025 272 / 0.45)', border:`1px solid ${T.border}`, borderRadius:10, padding:'10px 12px' }}>
            <p style={{ fontSize:'0.66rem', fontWeight:800, textTransform:'uppercase', letterSpacing:'0.1em', color:T.textMut, marginBottom:4 }}>Beginner note</p>
        ...
        <div style={{ color:cat.color, fontSize:'0.9rem', fontWeight:700, display:'flex', alignItems:'center', gap:4, marginTop:4 }}>
          {isTop ? 'Build your plan' : 'Explore'} <span style={{ transform: hov ? 'translateX(4px)' : 'none', transition:'transform 0.15s', display:'inline-block' }}>→</span>
        </div>
      </div>
    </div>
  )
