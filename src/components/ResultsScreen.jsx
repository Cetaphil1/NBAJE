...
const DISC_CATS = [
  ...
  { key:'lowBudget', label:'💸 Easy start',     color:T.blue,   dimColor:T.blueDim   },
  { key:'solo',      label:'🧍 Solo lane',      color:T.amber,  dimColor:T.amberDim  },
  { key:'social',    label:'👥 Find your people', color:T.accent, dimColor:T.accentDim },
    ...
    <div style={{ marginTop:48 }}>
      <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:6, gap:12, flexWrap:'wrap' }}>
        <div>
          ...
          <h3 style={{ fontSize:'1.4rem', fontWeight:800, letterSpacing:'-0.02em', color:T.textPri }}>The weird aisle.</h3>
        </div>
      ...
      <p style={{ fontSize:'0.92rem', color:T.textMut, marginBottom:18, lineHeight:1.55 }}>
        Not default quiz picks, but exactly the kind of thing someone becomes obsessed with.
      </p>
...
export default function ResultsPage({ initialTags, personalised, onSelect, onBack, onBobHint }) {
  ...
  return (
    <div style={{ width:'100%', maxWidth:660, margin:'0 auto', padding:'40px 16px 80px', position:'relative', zIndex:1 }}>
      ...
      <p style={{ fontSize:'1rem', color:T.textMut, marginBottom: personalised ? 20 : 28 }}>Pick one tiny experiment. You are not choosing your identity, just your next attempt.</p>
