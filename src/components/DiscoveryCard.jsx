      ...
      <div style={{ padding:'16px 20px 20px', display:'flex', flexDirection:'column', gap:12 }}>
        ...
        {meta.socialProof && (
          <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
            <p style={{ fontSize:'0.68rem', fontWeight:800, letterSpacing:'0.1em', textTransform:'uppercase', color:cat.color }}>Beginner read</p>
            <p style={{ fontSize:'0.82rem', color:T.textMut, lineHeight:1.55 }}>{meta.socialProof}</p>
          </div>
        )}
        {meta.actionBridge && (
        ...
        <div style={{ color:cat.color, fontSize:'0.9rem', fontWeight:700, display:'flex', alignItems:'center', gap:4, marginTop:4 }}>
          Build a first plan <span style={{ transform: hov ? 'translateX(4px)' : 'none', transition:'transform 0.15s', display:'inline-block' }}>→</span>
        </div>
      </div>
    </div>
  )
