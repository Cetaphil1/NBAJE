  ...
  return (
    ...
    >
      ...
      <div style={{ background:`linear-gradient(135deg, ${g1} 0%, ${g2} 100%)`, padding: isTop ? '28px 24px 22px' : '20px 20px 18px', position:'relative', overflow:'hidden', minHeight: isTop ? 120 : 90 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:14 }}>
          ...
          <div>
            ...
            {isTop && <div style={{ fontSize:'0.8rem', color:'rgba(255,255,255,0.6)', marginTop:3 }}>Best match right now</div>}
          </div>
        </div>
      </div>
      ...
      <div style={{ padding:'16px 20px 20px', display:'flex', flexDirection:'column', gap:12 }}>
        ...
        {meta.socialProof && (
          <div style={{ background:'rgba(255,255,255,0.025)', border:`1px solid ${T.border}`, borderRadius:10, padding:'10px 12px' }}>
            <p style={{ fontSize:'0.68rem', fontWeight:800, color:T.textMut, textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:4 }}>Why it sticks</p>
        ...
        <div style={{ color:cat.color, fontSize:'0.9rem', fontWeight:700, display:'flex', alignItems:'center', gap:4, marginTop:4 }}>
          See the first step <span style={{ transform: hov ? 'translateX(4px)' : 'none', transition:'transform 0.15s', display:'inline-block' }}>→</span>
        </div>
      </div>
    </div>
  )
