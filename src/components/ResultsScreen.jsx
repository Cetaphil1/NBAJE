...
const DISC_CATS = [
  { key:'bestFit',   label:'⭐ Strongest lead', color:T.accent, dimColor:T.accentDim },
  { key:'lowBudget', label:'💸 Easy on budget', color:T.blue,   dimColor:T.blueDim   },
  { key:'solo',      label:'🧍 Start alone',    color:T.amber,  dimColor:T.amberDim  },
  { key:'social',    label:'👥 Built-in people', color:T.accent, dimColor:T.accentDim },
  { key:'wildcard',  label:'🎲 Worth a detour', color:T.pink,   dimColor:T.pinkDim   },
]
  ...
  return (
    <div style={{ marginTop:48 }}>
      ...
      <p style={{ fontSize:'0.92rem', color:T.textMut, marginBottom:18, lineHeight:1.55 }}>
        These are off the main path, but sometimes the oddball option is the one that sticks.
      </p>
  ...
  return (
    <div style={{ width:'100%', maxWidth:660, margin:'0 auto', padding:'40px 16px 80px', position:'relative', zIndex:1 }}>
      ...
      <h2 style={{ fontSize:'clamp(1.8rem, 5vw, 2.6rem)', fontWeight:800, letterSpacing:'-0.02em', color:T.textPri, marginBottom:6 }}>
        A few sports that<br/>might actually fit.
      </h2>
      <p style={{ fontSize:'1rem', color:T.textMut, marginBottom: personalised ? 20 : 28 }}>Pick one that makes you curious. This is a first experiment, not a lifetime decision.</p>
