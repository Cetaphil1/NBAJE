...
const DISC_CATS = [
  { key:'bestFit',   label:'⭐ Best match',       color:T.accent, dimColor:T.accentDim },
  { key:'lowBudget', label:'💸 Low-cost start',   color:T.blue,   dimColor:T.blueDim   },
  { key:'solo',      label:'🧍 Good solo first',  color:T.amber,  dimColor:T.amberDim  },
  { key:'social',    label:'👥 People included',  color:T.accent, dimColor:T.accentDim },
  { key:'wildcard',  label:'🎲 Wildcard energy',  color:T.pink,   dimColor:T.pinkDim   },
]
    ...
    <div style={{ marginTop:48 }}>
      <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:6, gap:12, flexWrap:'wrap' }}>
        <div>
          ...
          <h3 style={{ fontSize:'1.4rem', fontWeight:800, letterSpacing:'-0.02em', color:T.textPri }}>Or follow the weird little spark.</h3>
        </div>
      ...
      <p style={{ fontSize:'0.92rem', color:T.textMut, marginBottom:18, lineHeight:1.55 }}>
        These are not default quiz picks. They are the side doors: specific, memorable, and sometimes exactly right.
      </p>
  ...
  return (
    <div style={{ width:'100%', maxWidth:660, margin:'0 auto', padding:'40px 16px 80px', position:'relative', zIndex:1 }}>
      ...
      <h2 style={{ fontSize:'clamp(1.8rem, 5vw, 2.6rem)', fontWeight:800, letterSpacing:'-0.02em', color:T.textPri, marginBottom:6 }}>
        Start with one good<br/>experiment.
      </h2>
      <p style={{ fontSize:'1rem', color:T.textMut, marginBottom: personalised ? 20 : 28 }}>No life sentence here. Pick the option that makes you curious enough to try once.</p>
