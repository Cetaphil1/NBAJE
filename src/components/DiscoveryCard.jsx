  ...
  return (
    ...
    >
      ...
      <div style={{ background:`linear-gradient(135deg, ${g1} 0%, ${g2} 100%)`, padding: isTop ? '28px 24px 22px' : '20px 20px 18px', position:'relative', overflow:'hidden', minHeight: isTop ? 120 : 90 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:14 }}>
          ...
          <div>
            ...
            {isTop && <div style={{ fontSize:'0.8rem', color:'rgba(255,255,255,0.6)', marginTop:3 }}>Your strongest match today</div>}
          </div>
        </div>
      </div>
      ...
      <div style={{ padding:'16px 20px 20px', display:'flex', flexDirection:'column', gap:12 }}>
        ...
        {meta.socialProof && (
          <p style={{ fontSize:'0.8rem', color:T.textMut, lineHeight:1.5 }}><strong style={{ color:T.textSec }}>Why it clicks:</strong> {meta.socialProof}</p>
        )}
        {meta.actionBridge && (
          <div style={{ display:'flex', alignItems:'flex-start', gap:8, background:T.surfaceHi, borderRadius:10, padding:'10px 14px', border:`1px solid ${T.border}` }}>
            ...
            <p style={{ fontSize:'0.88rem', color:T.textSec, lineHeight:1.5 }}><strong>Tiny first move:</strong> {meta.actionBridge}</p>
          </div>
        )}
